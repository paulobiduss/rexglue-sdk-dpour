...
// D3D12-only escape hatches (dpour-fork): a renderer that already records raw
// D3D12 on its own queue only needs the underlying objects to hand its
// finished frame to the guest output - not the whole nrhi surface. The
// deferred command list is the command processor's; recording into it is only
// valid inside the guest-output render callback.
ID3D12Resource* NativeRhiGetD3D12TextureResource(nrhi::Texture* texture);
DeferredCommandList* NativeRhiGetDeferredCommandList(nrhi::Device* device);
